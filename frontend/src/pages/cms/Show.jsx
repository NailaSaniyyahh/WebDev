...
const Show = () => {
  ...
  const [selectedShowCount, setSelectedShowCount] = useState(10);
  ...
  const filteredMovies = movies
    .filter((movie) => {
      ...
      const matchesSearchQuery = searchQuery
        ...
        : true;
      return (
        matchesCountry && matchesYear && matchesGenre && matchesSearchQuery
      );
    });
  ...
  return (
    <div className="tw-flex tw-justify-center tw-items-center tw-bg-gray-100 tw-min-h-screen tw-w-full tw-px-4">
      <div className="tw-bg-white tw-mt-4 tw-rounded-lg tw-shadow-lg tw-w-full lg:tw-max-w-6xl tw-overflow-hidden">
        ...
        <div className="tw-flex tw-flex-col sm:tw-flex-row tw-justify-between tw-items-center tw-px-6 tw-py-4 tw-border-t tw-border-b tw-border-gray-200">
        ...
        {/* Movies Table */}
        <div className="tw-overflow-x-auto tw-px-6 tw-py-4">
          <table className="tw-min-w-full tw-bg-white tw-border tw-border-gray-300 tw-text-sm sm:tw-text-base">
            ...
            <tbody>
              {moviesToShow.map((movie, index) => (
                <tr key={movie.id} className={index % 2 === 0 ? "tw-bg-white" : "tw-bg-gray-50"}>
                  <td className="tw-px-4 tw-py-2 tw-border-b tw-border-gray-300">{startIndex + index + 1}</td>
                  ...
                  <td className="tw-px-4 tw-py-2 tw-border-b tw-border-gray-300">{movie.countries.map((country) => country.name).join(", ")}</td>
                  <td className="tw-px-4 tw-py-2 tw-border-b tw-border-gray-300">{movie.actors.map((actor) => actor.name).join(", ")}</td>
                  <td className="tw-px-4 tw-py-2 tw-border-b tw-border-gray-300">{movie.genres.map((genre) => genre.name).join(", ")}</td>
                  ...
                  <td className="tw-px-4 tw-py-2 tw-border-b tw-border-gray-300">
                    <a href="#" className="tw-text-blue-600 hover:tw-underline" onClick={() => handleViewClick(movie)}>View</a>
                    <span className="tw-text-gray-500 tw-px-2">|</span>
                    <a href="#" className="tw-text-red-600 hover:tw-underline">Delete</a>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="tw-px-6 tw-py-3 tw-border-t tw-border-gray-200 tw-bg-white tw-flex tw-items-center tw-justify-between">
          <div className="tw-flex tw-flex-1 tw-justify-between sm:tw-hidden">
            <button onClick={handlePreviousPage} className="tw-relative tw-inline-flex tw-items-center tw-rounded-md tw-border tw-border-gray-300 tw-bg-white tw-px-4 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-700 hover:tw-bg-gray-50">Previous</button>
            <button onClick={handleNextPage} className="tw-relative tw-ml-3 tw-inline-flex tw-items-center tw-rounded-md tw-border tw-border-gray-300 tw-bg-white tw-px-4 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-700 hover:tw-bg-gray-50">Next</button>
          </div>
          <div className="tw-hidden sm:tw-flex sm:tw-flex-1 sm:tw-items-center sm:tw-justify-between">
            <p className="tw-text-sm tw-text-gray-700">
              Showing {startIndex + 1} to {Math.min(endIndex, totalMovies)} of {totalMovies} results
            </p>
      ...
      {isModalOpen && selectedDrama && (
        <div className="tw-fixed tw-inset-0 tw-bg-black tw-bg-opacity-50 tw-flex tw-items-center tw-justify-center tw-z-50 tw-p-4">
          <div className="tw-bg-white tw-p-6 tw-rounded-lg tw-shadow-lg tw-max-w-full tw-w-full md:tw-max-w-4xl lg:tw-max-w-5xl tw-relative tw-overflow-y-auto tw-max-h-screen">
            <button className="tw-absolute tw-top-2 tw-right-2 tw-bg-red-600 tw-text-white tw-px-3 tw-py-1 tw-rounded-lg tw-text-lg hover:tw-bg-red-700 focus:tw-outline-none" onClick={handleCloseModal}>
              X
            </button>
            ...
            <div className="tw-flex tw-flex-col md:tw-flex-row tw-space-y-4 md:tw-space-y-0 md:tw-space-x-4">
              ...
              <div className="tw-flex-grow">
                ...
                <p className="tw-mt-4">
                  Countries:{" "}
                  {selectedDrama.countries.map((country) => country.name).join(", ")}
                </p>
                <p className="tw-mt-4">
                  Actors:{" "}
